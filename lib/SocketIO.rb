...
module SocketIO
  ...
  class Client
    ...
    def send_connect
        @transport.send("1::#{@namespace}")
    end

    def send_heartbeat
      @transport.send("2::#{@namespace}") #rescue false
    end

    def send_message(string)
      @transport.send("3::#{@namespace}:#{string}") #rescue false
    end
    alias :send :send_message

    def send_json_message(hash)
      @transport.send("4::#{@namespace}:#{hash.to_json}") # rescue false
    end

    def send_event(name, hash)
      @transport.send("5::#{@namespace}:#{{name: name, args: [hash]}.to_json}") # rescue false
    end
